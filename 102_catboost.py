...
for fold, (trn_ind, val_ind) in enumerate(kfold.split(train[features], y)):
    ...
    with open(f"trained_models/catboost_{fold}.pkl", 'wb') as file:
        pickle.dump(clf, file)


print(f"CV Score: {amex_metric_mod(y, oof)}")
# CV Score: 0.7966888263472558
sub = pd.read_csv("input/sample_submission.csv")
sub['prediction'] = preds
sub.to_csv("output/submission_catboost.csv", index=False, header=True)

oof_df = pd.read_csv("input/train_labels.csv")
oof_df['prediction'] = oof
oof_df[['customer_ID','prediction']].to_csv("output/validation_catboost.csv", index=False, header=True)