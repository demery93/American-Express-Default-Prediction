...
features = np.load(f"feature_selection/top_1500_features.npy")
...
sub.to_csv(f"output/submission_model3.csv", index=False, header=True)

oof_df = pd.read_csv("input/train_labels.csv")
oof_df['prediction'] = oof
oof_df[['customer_ID', 'prediction']].to_csv(f"output/validation_model3.csv", index=False, header=True)
