...
gc.collect()
NFOLD = config.NFOLDS
params = config.lightgbm_dart_params[str(nfeats)]
...
for fold, (trn_ind, val_ind) in enumerate(kfold.split(train[features], y)):
    ...
    with open(f"trained_models/lgb_dart_{fold}_{nfeats}.pkl", 'wb') as file:
...
sub.to_csv(f"output/submission_lgb_dart_{nfeats}.csv", index=False, header=True)

oof_df = pd.read_csv("input/train_labels.csv")
oof_df['prediction'] = oof
oof_df[['customer_ID','prediction']].to_csv(f"output/validation_lgb_dart_{nfeats}.csv", index=False, header=True)